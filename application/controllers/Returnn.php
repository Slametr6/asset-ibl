...
class Returnn extends CI_Controller {
	...
	public function index()
	{
		...
		if ($username == '') {
			...
			if ($user['role_id'] == 1) {
				...
				$this->load->view('manager/form-return', $data);
				$this->load->view('include/footer');
			} else {
				$data['menu'] = 'return';
				$data['title'] = 'Form Return Managements';
	...
	public function exportExcel()
	{
		...
		$tanggal = date('d-m-Y');
		$excel->setActiveSheetIndex(0)->setCellValue('A1', "DATA PENGEMBALIAN ASSET - ".$tanggal); // Set kolom A1 dengan tulisan "DATA SISWA"
		...
		$excel->setActiveSheetIndex(0)->setCellValue('A2', "Laporan Data Pengembalian Asset IT PT Iron Bird"); // Set kolom A1 dengan tulisan "DATA SISWA"
		...
		$excel->setActiveSheetIndex(0)->setCellValue('L4', "Receiver");
		...
		$form_return = $this->m_return->getReturn();
		$no = 1; // Untuk penomoran tabel, di awal set dengan 1
		$numrow = 5; // Set baris pertama untuk isi tabel adalah baris ke 4
		foreach($form_return as $data){ // Lakukan looping pada variabel siswa
			...
			$excel->setActiveSheetIndex(0)->setCellValue('L'.$numrow, $data->receiver);
		...
		$excel->getActiveSheet(0)->setTitle("Laporan Data Pengembalian Asset");
		$excel->setActiveSheetIndex(0);
		// Proses file excel
		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment; filename="Data Pengembalian Asset.xlsx"'); // Set nama file excel nya
