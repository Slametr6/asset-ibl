...
class Asset extends CI_Controller {
	...
	public function expExcel()
	{
		...
		$excel->getProperties()->setCreator('My Notes Code')
							->setLastModifiedBy('My Notes Code')
							->setTitle("Data User Asset")
							->setSubject("User Asset")
							->setDescription("Laporan Semua Data User Asset")
							->setKeywords("Data User Asset");
		// Buat sebuah variabel untuk menampung pengaturan style dari header tabel
		$style_col = [
		  	'font' => ['bold' => true], // Set font nya jadi bold
			'alignment' => [
		...
		$tanggal = date('d-m-Y');
		$excel->setActiveSheetIndex(0)->setCellValue('A1', "DATA USER ASSET - ".$tanggal); // Set kolom A1 dengan tulisan "DATA SISWA"
		$excel->getActiveSheet()->mergeCells('A1:I1'); // Set Merge Cell pada kolom A1 sampai E1
		$excel->getActiveSheet()->getStyle('A1')->getFont()->setBold(TRUE); // Set bold kolom A1
		$excel->getActiveSheet()->getStyle('A1')->getFont()->setSize(16); // Set font size 15 untuk kolom A1
		$excel->getActiveSheet()->getStyle('A1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); // Set text center untuk kolom A1

		$excel->setActiveSheetIndex(0)->setCellValue('A2', "Laporan Data User Asset IT PT Iron Bird"); // Set kolom A1 dengan tulisan "DATA SISWA"
		$excel->getActiveSheet()->mergeCells('A2:I2'); // Set Merge Cell pada kolom A1 sampai E1
		$excel->getActiveSheet()->getStyle('A2')->getFont()->setBold(TRUE); // Set bold kolom A1
		$excel->getActiveSheet()->getStyle('A2')->getFont()->setSize(12); // Set font size 15 untuk kolom A1
		$excel->getActiveSheet()->getStyle('A2')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); // Set text center untuk kolom A1
		...
		$excel->setActiveSheetIndex(0)->setCellValue('E4', "SN"); // Set kolom E4 dengan tulisan SN
		$excel->setActiveSheetIndex(0)->setCellValue('F4', "Desc"); // Set kolom F4 dengan tulisan Desc
		$excel->setActiveSheetIndex(0)->setCellValue('G4', "Employee"); // Set kolom G4 dengan tulisan Employee
		$excel->setActiveSheetIndex(0)->setCellValue('H4', "Dept"); // Set kolom H4 dengan tulisan Dept
		$excel->setActiveSheetIndex(0)->setCellValue('I4', "Branch"); // Set kolom F4 dengan tulisan Branch
		$excel->setActiveSheetIndex(0)->setCellValue('J4', "Location"); // Set kolom J4 dengan tulisan Location
		$excel->setActiveSheetIndex(0)->setCellValue('K4', "Qty"); // Set kolom K4 dengan tulisan Condition
		$excel->setActiveSheetIndex(0)->setCellValue('L4', "Unit"); // Set kolom L4 Iengan tulisan Status
		...
		// Panggil function view yang ada di SiswaModel untuk menampilkan semua data siswanya
		$asset = $this->m_asset->getUserAsset();
		$no = 1; // Untuk penomoran tabel, di awal set dengan 1
		$numrow = 5; // Set baris pertama untuk isi tabel adalah baris ke 4
		foreach($asset as $data){ // Lakukan looping pada variabel siswa
			$excel->setActiveSheetIndex(0)->setCellValue('A'.$numrow, $no);
			...
			$excel->setActiveSheetIndex(0)->setCellValue('E'.$numrow, $data->sn);
			$excel->setActiveSheetIndex(0)->setCellValue('F'.$numrow, $data->descript);
			$excel->setActiveSheetIndex(0)->setCellValue('G'.$numrow, $data->emp_name);
			$excel->setActiveSheetIndex(0)->setCellValue('H'.$numrow, $data->dept_id);
		...
		// Proses file excel
		header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment; filename="Data User Asset.xlsx"'); // Set nama file excel nya
		header('Cache-Control: max-age=0');
		$write = PHPExcel_IOFactory::createWriter($excel, 'Excel2007');
		$write->save('php://output');
	}
	...
	public function printPDF()
	{
        ...
        $pdf->SetFont('Arial','B',16);
        // mencetak string 
        $pdf->Cell(300, 8, 'DATA USER ASSET - '.$tanggal, 0, 1, 'L');
        $pdf->SetFont('Arial','B',12);
		$pdf->Cell(300, 8, 'Laporan Data User Asset IT PT Iron Bird', 0, 1, 'L');
		$pdf->SetAutoPageBreak(true, 0);
        // Memberikan space kebawah agar tidak terlalu rapat
        $pdf->Cell(10, 8,'',0, 1);
        $pdf->SetFont('Arial','B', 9);
        $pdf->Cell(10, 8, "No", 1, 0, 'C');
        $pdf->Cell(36, 8, "Category", 1, 0, 'C');
        $pdf->Cell(17, 8, "No EQ", 1, 0, 'C');
        $pdf->Cell(30, 8, "No Asset", 1, 0, 'C');
		$pdf->Cell(25, 8, "SN", 1, 0, 'C');
        $pdf->Cell(145, 8, "Desc", 1, 0, 'C');
        $pdf->Cell(30, 8, "Employee", 1, 0, 'C');
        $pdf->Cell(15, 8, "Dept", 1, 0, 'C');
        $pdf->Cell(17, 8, "Branch", 1, 0, 'C');
        $pdf->Cell(20, 8, "Location", 1, 0, 'C');
        $pdf->Cell(10, 8, "Qty", 1, 0, 'C');
		$pdf->Cell(15, 8, "Unit", 1, 1, 'C');
		
		$pdf->SetFont('Arial','', 9);
		$assets = $this->m_asset->getUserAsset();
		$no = 1;
        foreach ($assets as $asset){
            $pdf->Cell(10, 8, $no, 1, 0, 'C');
			$pdf->Cell(36, 8, $asset->category, 1, 0, 'C');
        	$pdf->Cell(17, 8, $asset->no_eq, 1, 0, 'C');
        	$pdf->Cell(30, 8, $asset->no_asset, 1, 0, 'C');
        	$pdf->Cell(25, 8, $asset->sn, 1, 0, 'C');
        	$pdf->Cell(145, 8, $asset->descript, 1, 0, 'C');
        	$pdf->Cell(30, 8, $asset->emp_name, 1, 0, 'C');
        	$pdf->Cell(15, 8, $asset->dept_id, 1, 0, 'C');
        	$pdf->Cell(17, 8, $asset->branch_id, 1, 0, 'C');
        	$pdf->Cell(20, 8, $asset->location, 1, 0, 'C');
        	$pdf->Cell(10, 8, $asset->qty, 1, 0, 'C');
			$pdf->Cell(15, 8, $asset->unit, 1, 1, 'C');
			// $pdf->Ln();
			$no++;
		...
		ob_end_clean();
		$pdf->Output('Data User Asset - '.$tanggal.'.pdf', 'I');
